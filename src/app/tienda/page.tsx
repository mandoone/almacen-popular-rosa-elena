...
export default function TiendaPage() {
  const usaCalendarioTest = process.env.NEXT_PUBLIC_APP_ENV === 'test';
  ...
  useEffect(() => {
    ...
    fetch('/api/aperturas/relevante', { cache: 'no-store' })
      .then(async (res) => {
        const json = await res.json().catch(() => null);
        if (!res.ok || !json?.ok) {
          throw new Error(json?.error || 'No se pudo consultar la apertura activa.');
        }
        setApertura(json.data?.apertura ?? null);
      })
      .catch((err) => {
        setApertura(null);
        setErrorApertura(err instanceof Error ? err.message : 'Calendario no disponible.');
      })
  ...
  const enviarPedido = async () => {
    if (enviando) return;
    if (usaCalendarioTest && !apertura) {
      alert('No hay una apertura activa con pedidos anticipados disponibles.');
      return;
    }
    ...
    try {
      ...
      const json = await res.json().catch(() => null);
      if (!res.ok || !json?.ok) {
        const msg = json?.error || 'No se pudo registrar el pedido. Intenta nuevamente.';
      ...
      alert('No se pudo conectar para registrar el pedido. Revisa tu conexión e intenta de nuevo.');
    } finally {
      setEnviando(false);
    }
  };
  ...
  const carritoProps: CarritoPanelProps = {
    ...
    enviando,
    pedidoHabilitado: !usaCalendarioTest || Boolean(apertura),
  };
